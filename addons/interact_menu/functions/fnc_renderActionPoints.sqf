...
private _fnc_renderNearbyActions = {
    ...
    private _nearestObjects = nearestObjects [ACE_player, ["All"], 13];
    {
        ...
        private _classActions = GVAR(ActHashMap) getOrDefault [typeOf _target, []];
        {
            private _action = _x;
            // Try to render the menu
            if ([_target, _action] call FUNC(renderBaseMenu)) then {
...
private _fnc_renderSelfActions = {
    ...
    // Iterate through base level class actions and render them if appropiate
    private _hashMap = GVAR(ActSelfHashMap);
    private _classActions = _hashMap get typeOf _target;
