...
if (isServer) then {
    ...
    if (_ring) then {
        ...
        [QEGVAR(fire,addFireSource), [_obj, _radius, _intensity, str _obj + "_cookoff"]] call CBA_fnc_serverEvent;
    };
};
...
[{
    ...
    if (_elapsedTime >= _time) exitWith {
        ...
        [QEGVAR(fire,removeFireSource), [str _obj + "_cookoff"]] call CBA_fnc_serverEvent;
